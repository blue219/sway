...
- Start is available while the camera and pose model initialise. To prevent a participant entering the camera frame from starting a movement automatically, each movement first waits for one `Neutral` prediction at 70% confidence before it can accumulate recognised time. It then earns time whenever the current movement is predicted with at least 70% confidence. Recognition can be continuous after this initial neutral stance; returning to neutral is not required. Gaps longer than 300 milliseconds pause the timer without clearing prior progress. After five seconds of recognised movement time, the next movement starts immediately. Each round contains Side Arm Raise, Standing March, Shallow Squat, Standing Side Bend, and Side Leg Lift in a random, non-repeating order.
