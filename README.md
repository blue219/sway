...
The seated catalog in `src/game.ts` currently contains Seated knee extension, Seated torso twist, Seated arm-opening, Seated marching, and Seated Overhead Press in a fixed order. Knee extension uses its dedicated model; the other four movements currently show their videos without pose recognition. When each model is ready, add its paths to `CameraPreview.tsx` and enable pose recognition for that movement.
